...
function generateAnEquation() {
    ...
    while (Number.isInteger(firstNumber / secondNumber) == false){
       firstNumber = Math.floor(Math.random() * (max + 1 - min) + min);
       secondNumber = Math.floor(Math.random() * (max + 1 - min) + min);
    }
